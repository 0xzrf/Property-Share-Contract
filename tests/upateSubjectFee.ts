...
describe("UPDATE_SUBJECT_FEE INSTRUCT TESTING", () => {
  ...
  const SUBJECT_FEE = 10

  const associatedTokenProgram = ASSOCIATED_TOKEN_PROGRAM_ID

  it("Successfully updates payment token", async () => {
    const vals = await getVals(provider.connection, program.programId);
    await program.methods.initProp(new anchor.BN(vals.ID), SUBJECT_FEE, new anchor.BN(1000), new anchor.BN(100)) // unique identifier, subject fee, multiplier, base_price
    .accountsStrict({
    ...
    // Update token
    await program.methods.updateSubjectFee(10)
    .accountsStrict({
        owner: vals.propertyOwner.publicKey,
        property: vals.property
    })
    ...
    expect(configAccount.subjectFeePercent.toString()).to.equal(SUBJECT_FEE.toString());
  });
